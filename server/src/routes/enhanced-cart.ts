...
export function registerEnhancedCartRoutes(app: Express) {
  
  // Get enhanced cart with totals and applied coupons
  app.get("/api/cart/enhanced", optionalAuth, async (req: any, res) => {
    try {
  ...
  app.post("/api/cart/enhanced/items", optionalAuth, async (req: any, res) => {
    try {
      const userId = req.user?.uid;
      ...
      if (userId) {
        ...
        await storage.updateCartItem(cartItemId, {
          quantity,
          // Add enhanced properties when storage supports them
        });
      }

      // Return updated cart
      return app.get("/api/cart/enhanced")(req, res);
      
    } catch (error) {
      console.error("Error adding item to enhanced cart:", error);
      res.status(500).json({ message: "Failed to add item to cart" });
    }
  });

  // Update cart item
  app.put("/api/cart/enhanced/items/:id", optionalAuth, async (req: any, res) => {
    try {
      const userId = req.user?.uid;
  ...
  // Remove cart item
  app.delete("/api/cart/enhanced/items/:id", optionalAuth, async (req: any, res) => {
    try {
      const userId = req.user?.uid;
      const itemId = req.params.id;
  ...
  // Apply coupon
  app.post("/api/cart/enhanced/coupons", optionalAuth, async (req: any, res) => {
    try {
      const { code } = req.body;
  ...
  // Remove coupon
  app.delete("/api/cart/enhanced/coupons/:code", optionalAuth, async (req: any, res) => {
    try {
  ...
  // Clear entire cart
  app.delete("/api/cart/enhanced", optionalAuth, async (req: any, res) => {
    try {
      const userId = req.user?.uid;
  ...
  // Validate cart
  app.post("/api/cart/enhanced/validate", optionalAuth, async (req: any, res) => {
    try {
  ...
  app.get("/api/cart/enhanced/shipping-options", optionalAuth, async (req: any, res) => {
    try {
